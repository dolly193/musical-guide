...
const { client, loginBot, GUILD_ID, DISCORD_CLIENT_ID, DISCORD_CLIENT_SECRET, SITE_URL, sendProofForVerification } = require('./index.js');
...
app.use(session({
    ...
    resave: false,
    saveUninitialized: false, // Otimização: não salva sessões para visitantes não logados
    cookie: {
        sameSite: 'lax',
        maxAge: 30 * 24 * 60 * 60 * 1000 // Define a duração do cookie para 30 dias
    },
}));
...
// Página para usuários que não estão no servidor
app.get('/join-server', (req, res) => {
    // DISCORD_SERVER_INVITE_URL deve ser definida no seu .env
    res.render('join-server', { invite_url: process.env.DISCORD_SERVER_INVITE_URL });
});
    ...
    try {
        const tokenResponse = await fetch('https://discord.com/api/oauth2/token', {
            ...
            body: new URLSearchParams({
                ...
                code: code,
                redirect_uri: `${SITE_URL}/auth/discord/callback`, // CORREÇÃO: Usar a variável SITE_URL
            }),
        ...
        // Verifica se o usuário é membro do servidor principal
        const isMember = userGuilds.some(guild => guild.id === GUILD_ID);
        if (!isMember) {
            return res.redirect('/join-server');
        }
