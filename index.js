 ...
 * @param {import('discord.js').User} adminUser O administrador que executou a ação.
 * @returns {{success: boolean, message: string}}
 */
async function processVerificationAction(action, context, adminUser) {
    const { type, orderId, channelId, userId, productId, productName } = context;
    const guild = await client.guilds.fetch(GUILD_ID);

    if (action === 'approve') {
        if (type === 'ticket') {
            const result = await processPaymentConfirmation(guild, channelId, productId, adminUser);
            return result;
        } else if (type === 'site') {
            // Usa a nova função para buscar o pedido sem precisar do ID do usuário
            const targetOrder = await getOrderByAdmin(orderId);
            if (!targetOrder) {
                console.error(`[ERRO APROVAÇÃO] Pedido do site com ID ${orderId} não encontrado.`);
                return { success: false, message: `Pedido do site com ID ${orderId} não encontrado.` };
            }
            ...
            await logAuditEvent(`SITE: Pedido ${orderId} de <@${userId}> APROVADO por ${adminUser.tag}.`);
        ...
        if (type === 'ticket') {
            ...
            await logAuditEvent(`PAGAMENTO RECUSADO: Comprovante no canal ${paymentChannel?.name || channelId} recusado por ${adminUser.tag}.`);
            ...
            await logAuditEvent(`SITE: Pedido ${orderId} de <@${userId}> RECUSADO por ${adminUser.tag}.`);
        ...
        if (type === 'ticket') {
            ...
            await logAuditEvent(`ENTREGA CONFIRMADA: Entrega para ${user.tag} (Produto: ${productName}) concluída por ${adminUser.tag}.`);
            ...
            await logAuditEvent(`SITE: Pedido ${orderId} de ${user?.tag || userId} marcado como ENTREGUE por ${adminUser.tag}.`);
...
client.on(Events.InteractionCreate, async (interaction) => {
    if (interaction.isChatInputCommand()) { // Lida com comandos de barra
        ...
        } else if (['approve', 'reject', 'deliver'].includes(action)) {
            // Verifica se quem clicou é administrador
            if (interaction.user.id !== OWNER_ID) {
                return interaction.reply({ content: '❌ Apenas o dono do bot pode usar estes botões.', ephemeral: true });
            }

            await interaction.deferUpdate();

            // Reconstrói o objeto de contexto a partir do ID do botão
            const [, type, ...contextParts] = id.split('_');
            let context = {};
            if (type === 'ticket') {
                context = { type, channelId: contextParts[0], userId: contextParts[1], productId: contextParts[2] };
            } else if (type === 'site') {
                context = { type, orderId: contextParts[0], userId: contextParts[1], productId: contextParts[2] };
            }

            // Adiciona o productName se ele existir (para o deliver)
            if (contextParts.length > 3) {
                context.productName = contextParts.slice(3).join('_');
            }

            const result = await processVerificationAction(action, context, interaction.user);

            // Edita a mensagem original para desabilitar os botões e mostrar o resultado
            const originalEmbed = interaction.message.embeds[0];
            const newEmbed = EmbedBuilder.from(originalEmbed)
                .setFooter({ text: `Ação '${action}' executada por ${interaction.user.tag}. | ${result.message}` });

            // Desabilita todos os botões da mensagem
...
async function sendProofForVerification(verificationData) {
    try {
        const { details, context } = verificationData;

        // Constrói o ID customizado com base no contexto
        const contextString = context.type === 'ticket'
            ? `${context.type}_${context.channelId}_${context.userId}_${context.productId}`
            : `${context.type}_${context.orderId}_${context.userId}_${context.productId}`;

        const approveButton = new ButtonBuilder()
            .setCustomId(`approve_${contextString}`)
            .setLabel('Aprovar Pagamento')
            .setStyle(ButtonStyle.Success);

        const rejectButton = new ButtonBuilder()
            .setCustomId(`reject_${contextString}`)
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`Um novo comprovante de **${details.userTag}** precisa de atenção.`)
            .addFields({ name: 'Produto', value: `${details.productName} (R$ ${details.productPrice})` })
...
async function sendDeliveryNotification(deliveryData) {
    try {
        ...
        // Constrói o ID customizado com base no contexto
        const contextString = context.type === 'ticket'
            ? `${context.type}_${context.channelId}_${context.userId}_${context.productId}_${context.productName}`
            : `${context.type}_${context.orderId}_${context.userId}_${context.productName}`;

        const deliverButton = new ButtonBuilder()
            .setCustomId(`deliver_${contextString}`)
            .setLabel('Marcar como Entregue')
            .setStyle(ButtonStyle.Primary);

        const row = new ActionRowBuilder().addComponents(deliverButton);

        const owner = await client.users.fetch(OWNER_ID);
        const embed = new EmbedBuilder()
            .setTitle('📦 Marcar Pedido como Entregue')
